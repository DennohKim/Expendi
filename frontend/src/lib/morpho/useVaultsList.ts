  ...
  useEffect(() => {
    ...
    async function fetchVaults() {
      try {
        ...
        const formattedVaults: Vault[] = vaultItems.map(
          (vault: VaultApiResponse) => {
            ...
            return {
              ...
              netApy,
              tvl,
              description: vault.metadata?.description || `Earn yield on ${
              ...
              metadata: vault.metadata ? {
                ...
                forumLink: vault.metadata.forumLink,
        ...
        // Filter for whitelisted vaults if requested
        let filteredVaults = onlyWhitelisted 
          ? formattedVaults.filter(vault => vault.whitelisted)
          : formattedVaults;

        // Filter by asset if specified
        if (assetFilter) {
          filteredVaults = filteredVaults.filter(
            vault => vault.asset.toLowerCase() === assetFilter.toLowerCase()
          );
        }
    ...
    fetchVaults();
  }, [sortBy, chainId, onlyWhitelisted, assetFilter]); // sortVaults is now inside useEffect
